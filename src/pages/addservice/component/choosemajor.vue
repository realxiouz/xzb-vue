  ...
  watch: {
    initBean: {
      handler: function(val, oldVal) {
        if (val != null) {
          if (val.public_subject_id > 0) {
            this.form.type = 3;
            this.handleSel(3);
            this.form.selSub = val.public.id;
          } else if (val.unified_id > 0) {
            this.form.type = 2;
            this.handleSel(2);
            this.form.selMajors = [
              val.unify.unifiedfy_id,
              val.unify.unified_id
            ];
          } else if (val.major_id > 0) {
            this.form.type = 1;

            this.handleSel(1); //获取全部的地区
            this.form.selArea = val.school.area_id;

            let p = {
              pid: val.school.area_id,
              type: 1
            };
            //添加loading
            this.loading = true;
            schools(p).then(res => {
              this.colleges = res;
              this.form.selCollege = val.school.school_id;
              let p = {
                pid: val.school.school_id,
                type: 2
              };
              schools(p).then(res => {
                this.institutions = res;
                this.form.selInstitution = val.school.yuan_id;
                let p = {
                  pid: val.school.yuan_id,
                  type: 3
                };
                schools(p).then(res => {
                  this.majors = res;
                  this.form.selMajor =val.school.zhuanye_id + "-" + val.school.major_id;
                  //去除loading
                  this.loading = false;
                });
              });
